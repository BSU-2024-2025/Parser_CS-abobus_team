...
public class Parser(string input)
{
    ...
    if (!ParseStringLiteral("="))
    {
      ...
      )
      {
        if (!isInitLocal)
        {
          // convert  x ..= v; >> x = x .. v;
          currentIndex -= 2;
          var oper = GetCurrentChar().ToString();
          currentIndex = prevIndex;
          ParseVariable(name);
  ...
  public bool HasGlobalVariable(string variable)
  {
  ...
  private bool ParseFunctionCall(string name)
  {
    ...
    while (!ParseStringLiteral(")"))
    {
      ...
      argc++;
      ParseExpression();
      commandList.AddEndExpression(currentIndex); //commandList.AddOperator(currentIndex, ")");
      if (ParseStringLiteral(")")) break;
      if (ParseStringLiteral(","))
      {
        continue;
